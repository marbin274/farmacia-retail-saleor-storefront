...
export const CategoryItem = ({ category, isSelected, loading, toogle }) => {
  ...
  return (
    ...
    >
      <S.CategoryItemRoot className="fa-flex">
        ...
        <S.CategoryName className="fa-capitalize fa-ml-2">
        ...
        <div className="fa-mt-4 fa-border-t fa-border-solid fa-border-neutral-medium">
          <ul>
            {category.children.edges.map(({ node: child }, index) => (
              <li
                key={index}
                className="fa-my-5 fa-text-neutral-dark fa-font-medium"
              >
                {child.name}
              </li>
            ))}
