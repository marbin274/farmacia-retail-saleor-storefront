...
export const CollectionPage: React.FC<ViewProps> = ({ match }) => {
  ...
  const { data: collectionCategories, loading: collectionCategoriesLoading } = useCollectionCategories({ id: collectionId });


  const categoryOptions = React.useMemo((): SortOptions => {
    return [{ label: COLLECTION_CATEGORY_FILTER_LABEL, value: null }].concat(
      maybe(() =>
        collectionCategories?.collection?.categories?.edges?.map(({ node }) => ({ label: node.name, value: node.id }))
        , [])
    );
